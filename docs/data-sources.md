...
| 分類 | 用途 | 展示策略 |
| --- | --- | --- |
| 現在 | 仍在上架、測試、維護或對當前能力敘事最重要的 project ecosystem。 | 首頁和 selected projects 的主體。 |
| 探索 | 還不是標準產品，或更像研究、pipeline、system experiment、writing/case study 的內容。 | 可展示為 exploration / case study，但不假裝是正式產品。 |
| 歷史 | 已下架、舊版、舊職設計輸出、早期項目或 predecessor。 | 不作主展示；可作 history、design archive、project evolution 證據。 |
...
| Mini Program & Consumer App Design | design archive / app design | 元の氣 Bread、Fanfan IM App、HotCashback、兩秒記賬等小程序/消費類 app 設計和產品推導；其中 Bread 和 Fanfan 已有公開 URL。 | UI/UX Design、App、Product Storytelling、部分公開。 |
...
這些項目目前不放入「現在 / 探索 / 歷史」三個分類。這不是說它們永遠不能用，而是現階段不需要進入網站資料模型。
...
| Mini Program & Consumer App Design | `2020年04月﹣元の氣小程序 Design Version 1c.key` / `元の氣 Bread` Notion | 44 | 小程序設計完整度高，包含會員、點單、積分、購物車、WEUI、微信 API 和開發交付；Notion 公開頁可作為 Project detail 的公開工作面。 |
...
| Current Project Design Evidence | `TracklyReborn Keynote Action Button.key` | 3 | 屬於現在 的設計素材，不放歷史 Project，但可作 TracklyReborn design evidence。 |
...
### Project Detail 的公開工作面

Project detail 頁的最後一段使用 `detail.surfaces`，不是 `detail.proof`。

`links` 和 `surfaces` 的分工如下：

- `links`：給 project card 和 detail aside 使用，是快速跳轉入口。
- `detail.surfaces`：給 detail 頁使用，是可公開 URL 的整理集合，每一個 URL 都需要說明「為什麼值得看」。
- 如果項目目前沒有可公開 URL，不應該硬造連結；使用 pending item 說明資料仍需 redaction / public excerpt。
- support tooling repo 不自動進入 `surfaces`；只有對訪客理解 project 有價值的公開 URL 才列入。
...
type PublicSurface = {
  label: string;
  kind: "Product" | "Source" | "Writing" | "Notion" | "Release notes" | "Distribution" | "Internal archive" | string;
  href?: string;
  role: string;
};
```
...
- 網站首頁應該優先展示「現在」，而不是一串 app。
- 「探索」可以作為 case study / lab / notes 出現，但視覺和文案上不能假裝是正式產品。
- 「歷史」不作主展示；適合放在 history、archive、design archive、project evolution 裡。
...
1. 確認三個分類是否準確：現在、探索、歷史。
2. 確認「現在」清單：Syncnext、eisonAI、threadBridge、TracklyReborn、aDict。
3. 確認「探索」清單：HLN Machine、chatgpt-history，以及是否還有其他 Notion/Drive/GitHub 裡的探索項目。
4. 確認「歷史」清單：SyncPlaylist、Nomad Drive、Games Price app、uSpace，以及 5 個設計 archive project。
